...
let EditorControlsImpl = function (object, domElement) {
    ...
    // Dynamically adjust near/far clipping planes based on camera distance to target
    /**
     *
     */
    function updateClippingPlanes() {
        var distance = object.position.distanceTo(center);
        object.near = Math.max(0.01, distance * 0.001);
        object.far = Math.max(1000, distance * 100);
    ...
    function onMouseDown(event) {
        ...
        if (event.button === 0 && !ctrlDown) {
            state = STATE.ROTATE;
        } else if (event.button === 1) {
            state = STATE.ZOOM;
        } else if (event.button === 2 || ctrlDown) {
            state = STATE.PAN;
        }
        ...
        domElement.addEventListener("mouseleave", onMouseUp, false);
    ...
    // Set initial clipping planes based on current camera position
    updateClippingPlanes();
};
