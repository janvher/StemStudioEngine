...
export const OBJLoader2 = function (manager) {
    ...
    this.manager = LoaderSupport.Validator.verifyInput(manager, THREE.DefaultLoadingManager);
    this.logging = {
        enabled: true,
        debug: false,
    };
...
OBJLoader2.prototype = {
     ...
     * @param materialPerSmoothingGroup
     */
    ...
    _setCallbacks: function (callbacks) {
        if (LoaderSupport.Validator.isValid(callbacks.onProgress))
            this.callbacks.setCallbackOnProgress(callbacks.onProgress);
        if (LoaderSupport.Validator.isValid(callbacks.onReportError))
            this.callbacks.setCallbackOnReportError(callbacks.onReportError);
        if (LoaderSupport.Validator.isValid(callbacks.onMeshAlter))
        ...
        if (LoaderSupport.Validator.isValid(callbacks.onLoadMaterials))
    ...
    _loadObj: function (resource, onLoad, onProgress, onError, onMeshAlter, useAsync) {
        ...
        // fast-fail
        if (!LoaderSupport.Validator.isValid(resource))
    ...
    _loadMtl: function (resource, onLoad, onProgress, onError, crossOrigin, materialOptions) {
        if (MTLLoader === undefined)
            console.error('"MTLLoader" is not available. "OBJLoader2" requires it for loading MTL files.');
        if (LoaderSupport.Validator.isValid(resource) && this.logging.enabled)
        ...
        var processMaterials = function (materialCreator) {
            ...
            if (LoaderSupport.Validator.isValid(resource) && scope.logging.enabled)
        ...
        // fast-fail
        if (
            !LoaderSupport.Validator.isValid(resource) ||
            !LoaderSupport.Validator.isValid(resource.content) && !LoaderSupport.Validator.isValid(resource.url)
        ) {
            processMaterials();
        } else {
...
OBJLoader2.Parser.prototype = {
    ...
    buildFace: function (faceIndexV, faceIndexU, faceIndexN) {
        ...
        if (this.useIndices) {
            var mappingName =
                faceIndexV + (faceIndexU ? "_" + faceIndexU : "_n") + (faceIndexN ? "_" + faceIndexN : "_n");
