...
LoaderSupport.ResourceDescriptor = function (url, extension) {
    ...
    if (urlParts.length >= 2) {
        this.path = LoaderSupport.Validator.verifyInput(
                ...
                for (i = 0; i < fileDesc.length && !found; i++) {
                    ...
                    if (resource.extension.toLowerCase() === triple.ext.toLowerCase()) {
                        if (triple.ignore) {
                            ...
                            if (!(resource.content instanceof ArrayBuffer || resource.content instanceof Uint8Array))
...
LoaderSupport.MeshBuilder.prototype = {
    ...
    setMaterials: function (materials) {
        var payload = {
            ...
            materials: {
                materialCloneInstructions: null,
                serializedMaterials: null,
                runtimeMaterials: this.validator.isValid(this.callbacks.onLoadMaterials)
    ...
    _setCallbacks: function (callbacks) {
        ...
        if (this.validator.isValid(callbacks.onReportError))
        ...
        if (this.validator.isValid(callbacks.onLoadMaterials))
...
LoaderSupport.WorkerSupport = function () {
    ...
    this.loaderWorker =
        typeof window !== "undefined"
            ? new LoaderSupport.WorkerSupport.LoaderWorker()
            : new LoaderSupport.WorkerSupport.NodeLoaderWorker();
};
...
LoaderSupport.WorkerSupport.prototype = {
    ...
    validate: function (functionCodeBuilder, parserName, libLocations, libPath, runnerImpl) {
        ...
        if (LoaderSupport.Validator.isValid(runnerImpl)) {
            if (this.logging.enabled)
                console.info('WorkerSupport: Using "' + runnerImpl.runnerName + '" as Runner class for worker.');
            ...
            if (this.logging.enabled)
                console.info(
                    'WorkerSupport: Using DEFAULT "LoaderSupport.WorkerRunnerRefImpl" as Runner class for worker.',
                );
            ...
            if (this.logging.enabled)
                console.info(
                    'WorkerSupport: Using DEFAULT "LoaderSupport.NodeWorkerRunnerRefImpl" as Runner class for worker.',
                );
        }
...
LoaderSupport.WorkerSupport.LoaderWorker.prototype = {
                    ...
                    if (this.runtimeRef.logging.enabled)
                        console.info(
                            "WorkerSupport [" +
                                ...
                                "]: Run is complete. Terminating application on ajax!",
                        );
                ...
                if (this.runtimeRef.terminateRequested) {
                    if (this.runtimeRef.logging.enabled)
                        console.info(
                            "WorkerSupport [" +
                                this.runtimeRef.runnerImplName +
                                "]: Run reported error. Terminating application on ajax!",
                        );
                    this.runtimeRef._terminate();
                }
                break;
    ...
    run: function (payload) {
        ...
        if (!LoaderSupport.Validator.isValid(this.callbacks.meshBuilder))
            throw 'Unable to run as no "MeshBuilder" callback is set.';
        if (!LoaderSupport.Validator.isValid(this.callbacks.onLoad))
    ...
    setTerminateRequested: function (terminateRequested) {
        ...
        if (
            ...
            LoaderSupport.Validator.isValid(this.worker) &&
            !LoaderSupport.Validator.isValid(this.queuedMessage) &&
            this.started
        ) {
...
LoaderSupport.WorkerSupport.NodeLoaderWorker.prototype = Object.create(
    LoaderSupport.WorkerSupport.LoaderWorker.prototype,
);
...
LoaderSupport.WorkerDirector = function (classDef) {
    console.info(
        "Using LoaderSupport.WorkerDirector version: " + LoaderSupport.WorkerDirector.LOADER_WORKER_DIRECTOR_VERSION,
    );
    this.logging = {
        enabled: true,
        debug: false,
...
LoaderSupport.WorkerDirector.prototype = {
    ...
    _deregister: function (supportDesc) {
        if (LoaderSupport.Validator.isValid(supportDesc)) {
            ...
            if (LoaderSupport.Validator.isValid(loaderCallbacks.onProgress))
