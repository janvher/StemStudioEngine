...
interface Vector3RowProps {
    label: string;
    value: Vector3Value;
    setValue: (value: Vector3Value) => void;
    disabled?: boolean;
    margin?: string;
    min?: Vector3Value;
    max?: Vector3Value;
...
export const Vector3Row: React.FC<Vector3RowProps> = ({label, value, setValue, disabled = false, margin, min, max}) => {
    const handleAxisChange = (axis: keyof Vector3Value, newValue: number) => {
            ...
            <Vector3Container>
                ...
                <AxisRow>
                    ...
                    <NumericInputRow
                        label=""
                        value={value.z}
                        setValue={newValue => handleAxisChange("z", newValue)}
                        disabled={disabled}
                        $margin="0"
                        min={min?.z}
                        max={max?.z}
