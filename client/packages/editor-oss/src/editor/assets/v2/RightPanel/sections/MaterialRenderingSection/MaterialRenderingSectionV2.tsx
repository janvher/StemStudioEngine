...
export const MaterialRenderingSectionV2 = () => {
    const {context, root} = useAssetResolutionContext();
    const app = global.app as EngineRuntime;
    const editor = app.editor!;
    const selected = editor?.getSelectedObject();
    const materialInfo = editor.component?.state.selectedMaterialInfo;
    ...
    const handleSettingsChange = (settings: IMaterialSettings, pathKey: string) => {
        ...
        if (!selected.userData.materialSettings) {
            selected.userData.materialSettings = {};
        }

        const map = selected.userData.materialSettings as IMaterialSettingsMap;
        map[pathKey] = settings;

        applyMaterialSettingsToSpecificMaterial(selected, map, pathKey, freshContext);
    };
    ...
    const TEXTURE_EFFECTS_NUMERIC = [
        ...
        },
        {
            ...
            tooltip: "Horizontal texture scroll speed for animated effects like water.",
        },
        {
            ...
            tooltip: "Vertical texture scroll speed for animated effects like waterfalls.",
        },
    ];

    const saveMaterialSettingsToUserData = (selected: THREE.Object3D, settings: typeof materialSettings) => {
        if (!currentPathKey) return;
        handleSettingsChange(settings, currentPathKey);
    };
    ...
    }, [editor?.selected, materialInfo, editor.component?.state.selectedMaterialInfo]);
    ...
    return (
        <div className="Section MaterialSection">
            <SelectRow
                ...
                $margin="0"
                label="Material Type"
                labelTooltip="Determines the shading model and available texture slots."
            ...
            >
                ...
                <PanelCheckbox
                    ...
                    tooltipText="Renders both front and back faces, preventing see-through on thin geometry."
                />
                <Separator
                    invisible
                    margin="8px 0 0"
                />
                {TEXTURE_EFFECTS_NUMERIC.map(({label, value, key, tooltip}) => (
                    <NumericInputRow
                        key={label}
                        label={label}
                        value={value}
            ...
            <div style={{width: "100%"}}>
                <RowTitle style={{justifyContent: "flex-start"}}>Texture Settings</RowTitle>
                <Separator
                    invisible
                    margin="12px 0 0"
                />
