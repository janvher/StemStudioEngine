...
export interface BehaviorOptions {
    ...
    /**
     * Primary engine handle. Pass under the `stem` key in new code.
     * `erth` is kept as a deprecated alias so legacy call sites don't
     * have to migrate in lockstep.
     */
    stem?: StemEngineInterface;
...
export class BehaviorBase implements Behavior {
    /**
     * Primary engine API handle. Behaviors access engine subsystems
     * (`ai`, `asset`, `camera`, `physics`, …) through this field:
     *
     *     this.stem.ai.gen.generate3dModel(...)
     *     this.stem.scene.add(obj)
     *
     * Aliased as `this.stemEngine` and `this.erth` for backward compatibility
     * with existing user-authored behaviors. All three references point at
     * the same object; there is no functional difference.
     */
    readonly stem: StemEngineInterface;

    /** Deprecation alias for {@link stem}. New code should use `this.stem`. */
    get erth(): StemEngineInterface {
        return this.stem;
    }

    /** Long-form alias for {@link stem}. New code should use `this.stem`. */
    get stemEngine(): StemEngineInterface {
        return this.stem;
    }
    readonly gameObject: GameObject;
    ...
    constructor(target: Object3D, id: string, options: BehaviorOptions) {
        // Accept either `options.stem` (preferred) or `options.erth` (legacy)
