...
type TripoRetargetRequest struct {
...
type TripoRetargetResponse struct {
	Code int `json:"code"`
	Data struct {
...
func init() {
	serverContext.Handle(http.MethodPost, "/api/AI/ObjectGeneration/Tripo/Retarget", handleRetarget, constants.User)
}

func handleRetarget(w http.ResponseWriter, r *http.Request) {
	...
	if req.Animation == "" {
		http.Error(w, "Missing animation", http.StatusBadRequest)
		return
	}

	req.Type = "animate_retarget"
	...
	var responseBody TripoRetargetResponse
