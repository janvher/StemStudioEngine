...
type PixelateRequest struct {
	Image            string `json:"assetId"`
	PixelGridSize    int    `json:"pixelGridSize"`
	RemoveNoise      bool   `json:"removeNoise"`
	RemoveBackground bool   `json:"removeBackground"`
}

func init() {
	serverContext.Handle(http.MethodPost, "/api/AI/ImageGeneration/Pixelate", handlePixelate, constants.User)
}
...
func handlePixelate(w http.ResponseWriter, r *http.Request) {
	...
	payload := map[string]interface{}{
		"image":            req.Image,
		"pixelGridSize":    req.PixelGridSize,
		"removeNoise":      req.RemoveNoise,
		"removeBackground": req.RemoveBackground,
	}

	resp, err := client.MakeRequest("POST", "/generate/pixelate", payload)
	if err != nil {
