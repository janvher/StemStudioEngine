...
func init() {
	serverContext.Handle(http.MethodPost, "/api/AI/ImageGeneration/TextToImage", handleTextToImage, constants.User)
}
...
func handleTextToImage(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := client.MakeRequest("POST", "/generate/txt2img", payload)
	if err != nil {
	...
	jobStatus, err := WaitForJobCompletion(jobId)
	if err != nil {
