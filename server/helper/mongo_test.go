...
func TestTraction(t *testing.T) {
	config, err := GetConfig("../config.toml")
	if err != nil {
		t.Skip("Skipping transaction test - config file not available:", err)
	}
	...
	db, err := NewMongo(config.Database.Connection, dbName)
	if err != nil {
		t.Skip("Skipping transaction test - database connection failed:", err)
	}
...
func TestMongo_CollectionExists(t *testing.T) {
	config, err := GetConfig("../config.toml")
	if err != nil {
		t.Skip("Skipping MongoDB collection test - config file not available:", err)
	}
	...
	db, err := NewMongo(config.Database.Connection, dbName)
	if err != nil {
		t.Skip("Skipping MongoDB collection test - database connection failed:", err)
	}
...
func TestMongo_Descriptor(t *testing.T) {
	config, err := GetConfig("../config.toml")
	if err != nil {
		t.Skip("Skipping MongoDB descriptor test - config file not available:", err)
	}

	db, err := NewMongo(config.Database.Connection, "GISCoreTest")
	if err != nil {
		t.Skip("Skipping MongoDB descriptor test - database connection failed:", err)
	}
