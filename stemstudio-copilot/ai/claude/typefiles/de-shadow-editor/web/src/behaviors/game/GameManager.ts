import {initGlyphNodeMaterials, initNodeMaterials} from "@ni2khanna/uikit";
...
import {AssetType, getAssetRevisionData} from "../../api/asset";
import {getImportRevisionMapFromScriptBundle, getLambdasFromScriptBundle} from "../../api/behavior";
import {updatePlayCount} from "../../api/getGames";
import {getLambdaRevisionData, getLambdasFromAssets, getLambdasListForScene} from "../../api/lambda";
import {emitRewardEvent, REWARD_EVENT_TYPES} from "../../api/rewards";
import {getUserAvatar} from "../../api/user";
import EngineRuntime from "../../EngineRuntime";
import {AssetRef, assetRefKey} from "../../asset-management/AssetRef";
import {
...
import {shouldShowDebuggerTooltip} from "../../editor/assets/v2/BehaviorEditor/breakpoints";
import {debugSessionManager} from "../../editor/assets/v2/DebuggerPopup/DebugSessionManager";
import {PhysicsConfig} from "../../editor/assets/v2/types/physics";
...
import {PrefabManager} from "../../prefab/PrefabManager";
import {SceneConfig} from "../../scene/SceneConfig";
import {isVFXAutoStartEnabled} from "../../services";
import {showToast} from "../../showToast";
import {buildNameAwareScriptImportContext, loadScriptImportRevisionMap} from "../../script-runtime/scriptImports";
import {CAMERA_TYPES, CharacterOptionsInterface, GAME_STATE, IFRAME_MESSAGES, ISoundSettings} from "../../types/editor";
...
import {LoadingMessages} from "../../utils/LoadingManager";
import {getLogger, LogLevel} from "../../utils/Logger";
import ObjectPicker, {IObjectPicker} from "../../utils/ObjectPicker";
...
import {ModelData} from "../../v2/pages/services";
...
class GameManager {
    ...
    get sceneHelpers(): THREE.Scene { return this.engine.sceneHelpers; }
    ...
    async create(
        ...
        if (!this.isEnabled) {
            ...
            // toast.warning(I18n.t('GameManager: scene is not a game'));
        ...
        const importContext = await buildNameAwareScriptImportContext(this.engine.editor?.sceneID, sceneContext);
    ...
    async getAvatar(): Promise<ModelData> {
        const userModel = await getUserAvatar();
        return userModel;
    }
    ...
    async onMessage(topic: string, data: any) {
        ...
        if (cmd === "start") {
            ...
            // Auto-launch debugger popup when breakpoints are set
    ...
    private async registerAssetBehaviorClass(assetRef: AssetRef): Promise<void> {
        ...
        const assetResolutionContext = getAssetResolutionContext(this.scene) || emptyAssetResolutionContext;
        const importContext = await buildNameAwareScriptImportContext(this.engine.editor?.sceneID, assetResolutionContext);
