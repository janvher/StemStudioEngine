#!/usr/bin/env python3
"""Search for audio assets in the Studio 3D library."""
import sys, os; sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..', '_lib'))
from studio import studio_get, add_session_args, output
import argparse

parser = argparse.ArgumentParser(description="Search for audio assets in the Studio 3D library")
add_session_args(parser)
parser.add_argument("--phrases", required=True, nargs="+", help="Search phrases that describe the desired sound (e.g. --phrases coin pickup chime)")
